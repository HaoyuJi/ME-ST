...
class ScoreMeter(object):
    ...
    def update(
        ...
        n_correct = 0
        n_frames = len(pred)
        self.confusion_matrix += self._fast_hist(pred.flatten(), gt.flatten())
        self.n_videos += 1
        ...
        return round(acc,3), round(edit_score,3), f1s
    ...
    def get_scores(self) -> Tuple[float, float, float]:
        ...
        for i in range(len(self.iou_thresholds)):
            ...
            precs.append(round(precision,4))
        ...
        return round(acc,4), round(edit_score,4), f1s, precs, recalls
...
class BoundaryScoreMeter(object):
    ...
    def update(self, pred, gt):
        ...
        Args:
            preds: np.array. the model output(T)
            gts: np.array. boudnary ground truth array (T)
        Return:
            Accuracy
            Boundary F1 Score
        """
    ...
    def update_back(self, preds, gts, masks):
        """
        Args:
    ...
    def get_scores(self):
        ...
        # Accuracy, Edit Distance, F1 Score
        return round(acc,4), round(precision * 100,3), round(recall * 100,3), round(f1s,3)
