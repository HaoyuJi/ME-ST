...
from models.net_utils.PyramidTransformer import Encoder_bound
from models.net_utils.PyramidTransformer import Decoder
import math

from models.net_utils.TCNStage import SingleStageTCN
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = Encoder_bound(num_layers=args.num_layers, r1=2, r2=2, num_f_maps=args.num_f_maps,
        ...
        self.tcns = nn.ModuleList([SingleStageTCN(
            num_layers=args.num_layers, input_dim=1, num_f_maps = args.num_f_maps,
            num_classes=1, dilations=[2**i for i in range(args.num_layers)])
    ...
    def forward(self, x, mask):
        ...
        # forward
        x = self.conv_1x1(x)
        out, bound, feature = self.encoder(x, mask)
        ...
        for tcn in self.tcns:
            bound = tcn(self.activation_brb(bound), mask)
            bounds.append(bound)
