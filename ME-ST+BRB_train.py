...
import argparse
from Tools.postprocess import PostProcessor

parser = argparse.ArgumentParser()
# general arguments
parser.add_argument('--dataset', default='hugadb', type=str, help='dataset root path')
parser.add_argument('--netname', default='ME-ST+BRB', type=str, help='MAE-ST or MSGCN')
...
bce = nn.BCEWithLogitsLoss(pos_weight=trainset.pos_weight)
# endregion
...
def train():
    ...
    for data in tqdm(train_dataloader, file=sys.stdout):
        ...
        bound_label = data['boundary'].to(device)
        mask = data['mask'].to(device)
        # print("device:", next(net.parameters()).device)
        # print("device:",features.device,label.device,mask.device)
        optimizer.zero_grad()
        output_list, bound_list = net(features, mask)
        loss = 0
        for idx, (prob, bound) in enumerate(zip(output_list, bound_list)):
            ...
            b_loss = 0
            for b, bl, m in zip(bound, bound_label.unsqueeze(dim=1), mask[:, 0:1, :].bool()):
                b_loss += bce(b[m], bl[m])
            _loss += 0.1 * b_loss / bound.shape[0]
...
def eval():
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_dataloader):
            ...
            b = data['boundary'].to(device)
            mask = data['mask'].to(device)
            names = data['names']
            predictions, bounds = net(x, mask)
            ...
            for i in range(batch_size):
                ...
                if evaled.__contains__(name):
                    ...
                    evaled.append(name)
                    tobeshown = []
                    for idx, (prob, bound) in enumerate(zip([prediction_item[i] for prediction_item in predictions],
                                                                [bound_item[i] for bound_item in bounds])):
                        prob1 = prob.unsqueeze(dim=0).to("cpu").data.numpy()
                        bound1 = bound.unsqueeze(dim=0).to("cpu").data.numpy()  # 将结果变成ndarray
                        mask1 = mask.bool().to("cpu").data.numpy()
                        mask2 = mask1[i:i + 1, 0:1, :]
                        refined_output_cls = postprocessor(prob1, boundaries=bound1,
                                                           masks=mask2)  # 加上了边界的预测
                        predicted = np.squeeze(refined_output_cls)

                        target = t[i].cpu().numpy()

                        scores[idx].update(predicted, target)  # 通过预测和gt来更新各个分数 acc,edit,f1三种（0.1,0.25,0.5）
